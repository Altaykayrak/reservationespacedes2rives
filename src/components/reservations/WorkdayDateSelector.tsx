...
export const WorkdayDateSelector: React.FC<WorkdayDateSelectorProps> = ({
  ...
  // Validation du format de la classe scolaire
  const normalizedClass = childInfo.school_class.trim();
  if (!normalizedClass) {
    return (
      <EmptyHolidayState 
        message="Information invalide"
        subtitle="La classe de l'enfant n'est pas dans un format valide."
      />
    );
  }
